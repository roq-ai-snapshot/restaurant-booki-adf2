...
type ColumnType = ColumnDef<MenuItemInterface, unknown>;

interface MenuItemListPageProps {
...
export function MenuItemListPage(props: MenuItemListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MenuItemInterface>>(
    () => `/menu-items?params=${JSON.stringify(params)}`,
    fetcher,
  );
